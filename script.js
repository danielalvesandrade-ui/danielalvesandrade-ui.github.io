...
function calcularValores(bruto) {
    ...
    const isento65Ativo = document.getElementById('isento65').checked;
    const vIsento65 = isento65Ativo ? 1903.98 : 0;
    ...
    const bCalcIrrf = Math.max(0, bruto - sIrrf - vIsento65);
    const dLegal = vInss + (nDep * 189.59) + pensao, dUtil = Math.max(607.20, dLegal), bFinalI = Math.max(0, bCalcIrrf - dUtil);
    let aIr = 0, pIr = 0;
    ...
    return { vInss, vIrrf: irParc - redE, bInss, aI, dI, bCalcIrrf, dLegal, dUtil, bFinalI, aIr, pIr, redE, redC: red, pensao, outros, nDep, sIrrf, irParc, isento65Ativo, vIsento65 };
}
...
function calcular() {
    ...
    document.getElementById('resBaseRed').innerText = "R$ " + formatar(r.bCalcIrrf); 
    ...
    document.getElementById('ir1').innerText = formatar(b); 
    document.getElementById('ir2').innerText = formatar(r.sIrrf); 
    document.getElementById('memIsento65').innerText = r.isento65Ativo ? " - Isento 65+ (" + formatar(r.vIsento65) + ")" : "";
    document.getElementById('ir3').innerText = formatar(r.bCalcIrrf);
